...
//                         m_next[i] = m_next[i - 1] + 1; // 对上一个字符串提供帮助
//                     else {
//                         int cur = m_next[i - 1];
...
class kmp {
public:
    vector<int> getNext(const T c_String) {
        vector<int> m_next;
        m_next.resize(c_String.size());
        switch (c_String.size()) {
            case 0: {
    ...
    vector<int> nextToVal(const T c_String, const T& next) {
        vector<int> m_nextVal;
        m_nextVal.resize(next.size());
        m_nextVal[0] = {-1}; m_nextVal[1] = {0};
        for (int i = 2; i < next.size(); ++i) {
    ...
    int getIndexOfFirstletter(const T& o_String,
    const T& c_String,
    const vector<int>& next) {
        int i = 0, j = 0;
        while (i < o_String.size() && j < c_String.size()) {
